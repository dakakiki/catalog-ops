...
* Fixed: an operation that has already been undone no longer offers Undo. It could not have given anything back — every item would read as changed-since, so the safe policy would skip all of them and leave an empty operation in the history for a full pass over the catalogue. To put the change back, undo the undo.
